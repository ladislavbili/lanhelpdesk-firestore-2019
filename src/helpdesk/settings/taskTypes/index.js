      ...
      <div className="row">
        <div className="col-lg-4">
          ...
          <div className="table-responsive">
            ...
            <tbody>
              <tr className="clickable" onClick={()=>this.props.history.push('/helpdesk/settings/taskTypes/add')}>
                <td>+ Add task type</td>
              </tr>
              {this.state.taskTypes.filter((item)=>item.title.toLowerCase().includes(this.state.taskTypeFilter.toLowerCase())).map((taskType)=>
                <tr key={taskType.id} className="clickable" onClick={()=>this.props.history.push('/helpdesk/settings/taskTypes/'+taskType.id)}>
