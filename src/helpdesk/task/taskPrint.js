...
export default class TaskPrint extends Component {
  ...
  render() {
    return (
      <div className="display-inline print-icon">
        <ReactToPrint
          trigger={() => (
  ...
  render() {
    let taskWorks = this.props.taskWorks.map(work => {
      ...
      let totalPrice = (
        ...
        (1 - parseFloat(work.discount) / 100)
      ).toFixed(3);
      finalUnitPrice = finalUnitPrice.toFixed(3);
      let workType = this.props.workTypes.find(
        item => item.id === work.workType
      );
      let assignedTo = work.assignedTo
        ? this.props.users.find(item => item.id === work.assignedTo)
        : null;
    ...
    let taskMaterials = this.props.taskMaterials.map(material => {
      let finalUnitPrice =
        parseFloat(material.price) * (1 + parseFloat(material.margin) / 100);
      let totalPrice = (finalUnitPrice * parseFloat(material.quantity)).toFixed(
        3
      );
      finalUnitPrice = finalUnitPrice.toFixed(3);
      return {
      ...
      <div className="m-100">
        <div className="d-flex ">
          <div className="row flex">
            <h1 className="center-hor text-extra-slim">
                ...
                style={{
                  backgroundColor:
                    this.props.status && this.props.status.color
                      ? this.props.status.color
                      : "white"
                }}
              >
                {this.props.status ? this.props.status.title : "Neznámy status"}
              </span>
            </div>
          </div>
        </div>
        ...
        <div className="row">
          <div className="col-lg-12 d-flex">
            <p className="text-muted">
              Created by Branislav Šusta at{" "}
              {this.props.createdAt
                ? timestampToString(this.props.createdAt)
                : ""}
            </p>
            <p className="text-muted ml-auto">
              {this.props.statusChange
                ? "Status changed at " +
                  timestampToString(this.props.statusChange)
                : ""}
            </p>
          </div>
        </div>
        ...
        <div className="col-lg-12 row">
          ...
          {this.props.tags.length === 0 && (
            <div className="display-inline m-r-5">no tags</div>
          )}
        ...
        <div className="col-lg-12 row">
          ...
          {this.props.assignedTo.length === 0 && (
            <div className="display-inline m-r-5">no people were assigned</div>
          )}
        ...
        <div className="col-lg-12">
          ...
          <div className="col-lg-6">
            <div className="">
              ...
              <div className="row">
                <label className="col-5 col-form-label text-slim">
                  Deadline
                </label>
                <div className="col-7">
        ...
        {false && (
          <div className="form-group m-b-0 row">
            ...
            <div className="col-7">{this.props.overtime}</div>
          </div>
        )}
        {false && (
          <div className="row">
            ...
            <div className="col-7">{this.props.reminder}</div>
          </div>
        )}
