...
//		this.changeUserWrite.bind(this);
		...
		fetchData(id){
			...
			rebaseFirestore.get('tags/' + id, {
				context: this,
				withIds: true,
			}).then((tag) =>{
					 /*rebaseFirestore.get('/users', {
						context: this,
						withIds: true,
						}).then((users) =>*/
								this.setState({
  ...
  //  let newWrite = this.state.write.map(user => user.id);
    rebaseFirestore.updateDoc('/tags/'+this.props.match.params.tagID, {name:this.state.name, body:this.state.body, /*read: newRead, write: newWrite, public: this.state.public,*/ active: this.state.active})
    .then(() => {
      this.setState({
        value: 100,
				saving: false,
    ...
    if (window.confirm("Chcete zmazať tento tag?")) {
      rebaseFirestore.removeDoc('/tags/'+this.props.match.params.tagID)
        .then(() =>
            { rebaseFirestore.get('/notes', {
                context: this,
                withIds: true,
              })
              .then((notes) =>{
                ...
                .map(note =>
                    rebaseFirestore.updateDoc('/notes/'+note.id, {name: note.name, body: note.body, tags: note.tags.filter(item => item !== this.props.match.params.tagID)})
                  );
