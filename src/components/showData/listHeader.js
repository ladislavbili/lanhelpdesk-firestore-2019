    ...
    return (
      <div className="m-r-5 m-l-5 d-flex flex-row align-items-center">
        ...
        <div className="d-flex flex-row align-items-center ml-auto">
          <div className="text-basic m-r-5 m-l-5 sort-by-label">Sort by</div>
          <select
            value={this.props.orderBy}
            className="invisible-select sort-by"
          ...
          {!this.props.ascending && (
            <button
              type="button"
              className="btn btn-link btn-outline-blue waves-effect"
              onClick={() => this.props.setAscending(true)}
            >
              <i className="fas fa-arrow-up icon-M" />
            </button>
          )}
          ...
          {this.props.ascending && (
            <button
              type="button"
              className="btn btn-link btn-outline-blue waves-effect"
              onClick={() => this.props.setAscending(false)}
            >
              <i className="fas fa-arrow-down icon-M" />
            </button>
          )}
        </div>
      </div>
